...
class AternosAPI(StatefulBrowser):
    ...
    def login(self):
        self.setSec()
        data = self.post('https://aternos.org/panel/ajax/account/login.php',params={'user':'HackR27','password':"a881d0db56264a6f3e7650c243971f10",'SEC':self.sec},self.reqheaders)
        print(data.status_code)

    def setSec(self):
        key = self.keygen()
        value = self.keygen()
        cloud = self.getCookie()
        self.reqheaders['User-Agent'] = cloud[1]
        self.reqheaders['Cookie'] = f'ATERNOS_SEC_{key}={value}; '+cloud[0].split(' ')[0]
        self.sec = key+':'+value
    ...
    def getCookie(self):
        data = cloudscraper.get_cookie_string('https://aternos.org/:en')
        return data
